    ...
    <script>
        ...
        $(function () {
            ...
            $('#cbx_select_all').click(function () {
                ...
                $('input.cbx_select').prop('checked', checked);
            });
            ...
            $('#btn_product_delete').click(function () {
                ...
                if (confirm('确定要删除选中的产品吗？')) {
                    $.ajax({
                        url: 'admin/product/deleteBatch',
                        type: 'DELETE',
                        data: JSON.stringify(selectedIds), // 将数组转换为JSON字符串
                        contentType: 'application/json',
                        success: function (response) {
                            if (response.success) {
                                alert('产品删除成功！');
                                // 重新加载产品列表
                                getData(null, 'admin/product/0/10', dataList);
                            } else {
                                alert('产品删除失败：' + response.message);
                            }
                        },
...
<div class="frm_div text_info">
    ...
    <div class="frm_group">
        ...
        <input id="checkbox_product_isEnabled_special" name="checkbox_product_isEnabled" type="checkbox" value="2"
               checked>
        <label class="frm_label" id="lbl_product_isEnabled_special"
               for="checkbox_product_isEnabled_special">促销中</label>
    ...
    <div class="frm_group_last">
        ...
        <input class="frm_btn frm_delete" id="btn_product_delete" type="button" value="批量删除"/>
...
<div class="table_normal_div">
    <table class="table_normal" id="table_product_list">
        ...
        <c:forEach items="${requestScope.productList}" var="product">
            <tr>
                <td><input type="checkbox" class="cbx_select" id="cbx_product_select_${product.product_id}"
                           value="${product.product_id}"><label for="cbx_product_select_${product.product_id}"></label>
                </td>
                ...
                <td>
                    <span class="td_special" title="查看产品详情"><a href="javascript:void(0)"
